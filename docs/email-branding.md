...
`scripts/brand/build-marks.mjs`), the dawn-gradient top rule (the
brand's one moment of colour, with a solid-ink fallback for clients
that drop CSS gradients), an ink (`#14161A`) button, on a paper
(`#F4F5F6`) background. Emails are light-only. Palette + assets:
