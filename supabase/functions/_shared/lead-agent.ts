...
export function buildSystemPrompt(
  ...
  return [
    ...
    '- As soon as the prospect shares their name, call capture_lead. Pass their mobile number too if they give one — it is what anything you send them goes to, so on a call read it back digit by digit and let them correct you before you save it. Add useful details (goals, availability) as notes.',
...
async function textProspect(ctx: ToolContext, message: string): Promise<TextOutcome> {
  ...
  // A demo gym is exempt from needing one: its send is simulated (0278),
  // and the message still lands in the thread the visitor is reading.
