-- 0243: A cover offer you can take from the Timeline
--
-- The cover screen does three things: shows what is uncovered, lets a
-- coach ask for cover, and lets a coach claim it. `classes.uncovered`
-- took the first, and the ops job has been chasing the coaches since
-- 0208. This is the third, and it is the one the screen could not lose
-- without taking away the only way a coach can pick a class up.
--
-- The feed already carries `cover_requested` and already gates it on
-- can_claim_cover — exactly the coaches who could take it. What it
-- carried was a count, which is a thing to read rather than a thing to
-- do. It now carries the offers themselves.
--
-- Two filters on those offers, and both matter more than they look:
--
--   claimed_by is null   a claimed offer belongs to somebody else, and a
--                        button on it can only produce "already claimed"
--   starts_at > now()    a class that already ran cannot be covered, and
--                        offering it sends a coach to fail
--
-- Plus `requested_by`, so the card can leave the buttons off your own
-- request. claim_cover refuses that case with "You cannot cover your own
-- class" — correct, and a button whose only outcome is that message is a
-- button that should not have been drawn.
--
-- The claim itself is unchanged and stays first-come: claim_cover takes
-- the row FOR UPDATE, so two coaches tapping at once resolve in the
-- database and the loser is told. This migration does not make claiming
-- safer or fairer; it moves where the offer is seen.
