-- 0286: the proposal card gets one way in that works.
--
-- "Read and edit first" on a chase proposal looked its subscription up
-- from the client at press time and navigated only if it found one. When
-- the action carried no subject_subscription the tap did nothing at all:
-- no navigation, no error, no explanation. The seeded demo's proposals
-- are exactly that shape, so on the showcase tenant the chip was inert.
--
-- The card draws every other fact it shows from the feed, so the
-- subscription travels with it now too and the chip is either a link the
-- card knows it can follow or a link it doesn't offer — never a control
-- that swallows the tap.
--
-- Body taken from 0252, the latest definition. Same RETURNS shape and
-- arity, so CREATE OR REPLACE is safe; only the ledger branch's jsonb
-- gains a key.
  ...
  select * from (
    ...
    select 'action:' || a.id::text,
           ...
           jsonb_build_object(
             ...
             'payload', a.payload,
             'evidence', a.evidence,
             'subscription_id', a.subject_subscription
           )
