-- 0247: The line you can open
--
-- The Timeline says "Isla Irwin joined." and "Emma's payment didn't go
-- through" and stops there — a stream you can only read. The nudge lines
-- are the worst of it: "I've nudged Emma about their payment" is a claim
-- with its receipt locked in tables the screen never opens, so an owner
-- who wants to know what was actually said, whether it went, and what
-- came of it has nowhere to tap.
--
-- The story itself needs no new read path — agent_actions,
-- agent_outbound_messages and agent_cases have carried can_see_money
-- SELECT policies since 0204/0206, so the new detail screen reads them
-- under RLS like any other screen. What IS missing is the key: a
-- member_joined row carries an empty detail and a payment_failing row
-- names the plan but not the person, so neither line can open the
-- member it is about. Two ids, added to detail; same RETURNS shape, so
