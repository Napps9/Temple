-- Per-class-type booking-window overrides.
--
-- 0055 added gym-wide booking_window_hours_ahead /
-- booking_cutoff_minutes_before / cancel_cutoff_minutes_before. Real
-- gyms run a mix of formats: an open-gym slot might have no cutoff
-- ("turn up if there's space") while a coached class needs a strict
-- one-hour-before cut. This migration lets a class_type override any
-- of the three; NULL means "use the gym default" so existing class
-- types see no behaviour change.
--
-- _book_class_for and the refund trigger read the class_type
-- override first, falling back to gyms.* when the override is NULL.
...
alter table public.class_types
  ...
  add column if not exists booking_cutoff_minutes_before integer
    check (booking_cutoff_minutes_before is null
           or booking_cutoff_minutes_before >= 0),
  add column if not exists cancel_cutoff_minutes_before  integer
    check (cancel_cutoff_minutes_before is null
           or cancel_cutoff_minutes_before >= 0);

-- ============================================================================
-- _book_class_for: class-type override → gym default → unlimited
-- ============================================================================
...
-- ============================================================================
-- Refund trigger: class-type override → gym default → no cutoff
-- ============================================================================
