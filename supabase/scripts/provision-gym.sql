...
--   2. Edit the three variables at the top of the DO block below.
...
begin
  ...
  values (v_owner_id, split_part(v_owner_email, '@', 1))
  on conflict (id) do nothing;

  insert into public.gym_memberships (gym_id, profile_id, role, status)
  values (v_gym_id, v_owner_id, 'owner', 'active');

  raise notice 'Provisioned gym % (%) with owner %', v_gym_name, v_gym_id, v_owner_email;
end;
$$;
