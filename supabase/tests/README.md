...
| `a_price_that_holds_for_three_months.sql` | Plan coupons (0264, 0265): every refusal `preview_plan_coupon` can give (including a repeating discount on a one-off credit pack), an empty plan list meaning EVERY plan, another gym's code being indistinguishable from an unknown one, a cross-gym plan refused, the money fields frozen once Stripe holds the coupon (name still editable), `can_assign_plan` without `can_manage_plans` refused, members unable to list codes, and neither anon nor authenticated able to record a redemption. |
| `the_number_you_just_beat.sql` | `record_personal_best` (0263) writes nothing for a first-ever log or an equalled best, writes a milestone for a strictly better one in either direction, updates rather than duplicates within the same local day, refuses an unknown metric or direction in 0101's words, keeps milestones self-only under RLS, and grants anon nothing. |
| `a_link_that_books_a_free_class.sql` | Redeeming a trial pass (0262) books nothing — it holds the seat; the waiver and PAR-Q are still refused until signed by the claimant, and the booking then spends the comp grant at a gym that requires membership. Covers the comp-window trap (a pass for a distant class still covers it), a held seat counting against capacity for others but never its holder, idempotent re-redemption, a returning coach not being demoted, revoked/unknown tokens offering nothing, anon's grants, and staff-only RLS. |
