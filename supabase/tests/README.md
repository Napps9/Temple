...
| `a_price_that_holds_for_three_months.sql` | Plan coupons (0264): every refusal `preview_plan_coupon` can give, an empty plan list meaning EVERY plan, another gym's code being indistinguishable from an unknown one, a cross-gym plan refused, the money fields frozen once Stripe holds the coupon (name still editable), `can_assign_plan` without `can_manage_plans` refused, members unable to list codes, and neither anon nor authenticated able to record a redemption. |
