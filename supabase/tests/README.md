...
| `cover_uncovered_warning.sql` | `warn_uncovered_cover` warns the requester plus owners/admins about unclaimed classes inside the 48h lead — one digest per request, not per class; leaves far-off requests and other coaches alone; day-scoped idempotency key so a same-day re-run is a no-op but tomorrow warns again. |
