...
import { dateRangeWindow, validateDateRange } from './date-range';

describe('dateRangeWindow', () => {
  it('runs from local midnight to midnight the day after the end date', () => {
    expect(dateRangeWindow('2026-12-22', '2027-01-03', 'UTC')).toEqual({
  ...
  it('handles a single-day range', () => {
    expect(dateRangeWindow('2026-12-25', '2026-12-25', 'UTC')).toEqual({
  ...
  it('rolls the end date over a month boundary', () => {
    expect(dateRangeWindow('2026-01-28', '2026-01-31', 'UTC')?.endIso).toBe(
  ...
  it('shifts both edges for a summer BST window', () => {
    ...
    // day before.
    expect(dateRangeWindow('2026-07-06', '2026-07-10', 'Europe/London')).toEqual(
      {
  ...
  it('shifts only the start edge across the autumn DST change', () => {
    ...
    expect(dateRangeWindow('2026-10-24', '2026-10-28', 'Europe/London')).toEqual(
      {
  ...
  it('handles a positive-offset zone', () => {
    ...
    expect(
      dateRangeWindow('2026-12-22', '2026-12-23', 'Australia/Brisbane'),
    ).toEqual({
  ...
  it('rejects malformed and impossible dates', () => {
    expect(dateRangeWindow('22/12/2026', '2027-01-03', 'UTC')).toBeNull();
    expect(dateRangeWindow('2026-02-30', '2026-03-05', 'UTC')).toBeNull();
    expect(dateRangeWindow('', '2027-01-03', 'UTC')).toBeNull();
  });
});
...
describe('validateDateRange', () => {
  ...
  it('accepts a future range', () => {
    expect(
      validateDateRange('2026-12-22', '2027-01-03', 'UTC', now),
    ).toBeNull();
  });

  it('rejects an empty start or end', () => {
    expect(validateDateRange('', '2027-01-03', 'UTC', now)).toBe(
      'Pick a start date',
    );
    expect(validateDateRange('2026-12-22', '', 'UTC', now)).toBe(
      'Pick an end date',
    );
  });

  it('rejects an end before the start', () => {
    expect(validateDateRange('2027-01-03', '2026-12-22', 'UTC', now)).toBe(
      'The end date is before the start date',
    );
  });

  it('rejects a range wholly in the past', () => {
    expect(validateDateRange('2026-07-01', '2026-07-10', 'UTC', now)).toBe(
      'That date range is in the past',
    );
  });

  it('accepts a range ending today, since today has hours left', () => {
    expect(validateDateRange('2026-07-20', '2026-07-27', 'UTC', now)).toBeNull();
  });
});
