// Cover date-range window arithmetic, mirroring what
// request_cover_range does in Postgres: the window runs from local
// midnight on the start date to local midnight on the day AFTER the end
// date, in the gym's timezone. End-exclusive, so "to 3rd January"
// includes every class on the 3rd whatever time it runs.
//
// The preview list in CoverRangeModal has to agree with the RPC exactly
// or a coach sees one set of classes and gets another.
...
export function coverRangeWindow(
  startIso: string,
  endIso: string,
  tz: string,
...
export function validateCoverRange(
  startIso: string,
  endIso: string,
  tz: string,
  now: Date,
  ...
  const window = coverRangeWindow(startIso, endIso, tz);
